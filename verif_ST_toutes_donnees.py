...
class DecisionLeaf:
	...
	def find_attr_discriminant(self, data, mode=-1):
		nb_attr = data[0].size
		std = -1;
		selected_attr = -1;
...
data = np.array(data_df)
data = np.delete(data, 2, 1)
leaf2 = DecisionLeaf(int(data.size / data[0].size))
leaf2.find_attr_discriminant(data, 1)
size = np.delete(data, 1, 1).size
x = np.resize(np.delete(data, 1, 1), (1, size))[0]
y = np.resize(np.delete(data, 0, 1), (1, size))[0]
a = [x.tolist(), y.tolist()]
...
print(leaf2.seuilB)
for k in range(int(data.size/data[0].size)):
	if (data[k][0] <= leaf.seuilA):
		data[k][2] += 10
	elif (data[k][0] > leaf.seuilB):
		data[k][2] += 20
		
	if (data[k][0] <= leaf2.seuilA):
		data[k][2] += 100
	elif (data[k][0] > leaf2.seuilB):
		data[k][2] += 200
